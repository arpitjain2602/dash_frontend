...
def NamedSlider(name, id, min, max, step, value, marks=None):
    if marks:
        ...
        marks_array = np.arange(min, max, step)
        marks_array = list(marks_array)
        marks = {i: i for i in marks_array}
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                ...
                children=dcc.Slider(
                    id=id, min=min, max=max, step=step, value=value , marks=marks, 
                ),
            ),
        ],
    )
...
def NamedInlineInput(name, short, placeholder, type_input, val, **kwargs):
    return html.Div(
        id=f"div-{short}",
        style=_merge(
            {"display": "block", "margin-bottom": "5px", "margin-top": "5px"},
